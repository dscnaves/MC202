...
int catalago_de_doenças(Disease doencas[]){
    ...
    for (int i = 0; i<num_diseases; i++){
        fscanf(file_disease,"%s ", doencas[i].CID);
        fgets(doencas[i].nameDisease, 101, file_disease);
    }
...
int catalogo_de_medicos(Doctor medicos[]){
    ...
    for (int i = 0; i<num_doctors; i++){
        fscanf(file_doctor, "%s", medicos[i].CRM);
        fscanf(file_doctor, "%s[^\n]", medicos[i].nameDoctor);
...
void open_medical_clinic(ManagementMedicalAppointment *gerenciamento, int max_consultas){
    /*Essa função inicializa/"Abre" um dia da clínica*/
    gerenciamento->num_consultas = 0; //Inicializando o dia com 0 consultas registradas no início do dia
    gerenciamento->max_consultas = max_consultas;
}
...
void registrar(ManagementMedicalAppointment *gerenciamento, char *ponteiroNamePacient, char *CID, char *CRM, Disease doencas[], int num_diseases, Doctor medicos[], int num_doctors){
    //Acessando variáveis da struct que gerenciamento aponta
...
void close_medical_clinic(ManagementMedicalAppointment *gerenciamento){
    ...
    for (int i = 0; i<gerenciamento->num_consultas; i++){
        printf("%s atendido por %s: %s", gerenciamento->consultas[i].namePacient, gerenciamento->consultas[i].medico.nameDoctor, gerenciamento->consultas[i].doenca.nameDisease);
    }
}
...
int main(){
    ...
    while (scanf("%s", tarefa) != EOF){
        if (strcmp(tarefa, "abrir") == 0){
            ...
            char CID[4]; //CID: 1 letra + 2 dígitos + \0
