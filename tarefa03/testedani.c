...
Matrix * create_matrix(int n, int d){
    /*Função retorna um ponteiro que aponta para o local da memória onde os vetores estarão armazenados*/
    ...
    m->n = n; // Armazena o número de vetores (n) no campo n da estrutura Matrix apontada por m
    m->d = d; // Armazena a dimensão dos vetores (d) no campo d da estrutura Matrix apontada por m

    //Alocando o espaço para o array de vetores: Memória suficiente para armazenar as n estruturas Vector
    m->vetores = (Vector*) malloc(n*sizeof(Vector)); 
        if (m->vetores == NULL){
        printf("Erro de alocação da memória dos vetores!\n");
        free(m); // Libera a memória alocada para 'm' se a alocação dos vetores falhar
        exit(1);
    }
...
float modulus(Vector * v, int d){
    ...
    for (int i = 0; i<d; i++){
        num_aux = pow(v->original_vector[i], 2);
        soma_quadrados += num_aux;
    }

    return sqrt(soma_quadrados);
}

void max_min_normalize(Matrix * m, int idx, int d, int n){
    
    float x_min = m->vetores[idx].original_vector[0]; //O menor elemento começa o programa sendo o primeiro
    float x_max = m->vetores[idx].original_vector[0];

    for (int j = 1; j<d; j++){ //Percorrendo todas as coordenadas

        //Se o próximo elemento for menor do que x_min, então o próximo elemento assume a posição de x_min
